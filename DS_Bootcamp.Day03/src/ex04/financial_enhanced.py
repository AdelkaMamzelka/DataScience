...
#вводим команду в терминале 3) python -m cProfile -s time financial_enhanced.py > profiling-http.txt 'MSFT' 'Total Revenue'
# 4) python -m cProfile -s ncalls financial_enhanced.py > profiling-ncalls.txt 'MSFT' 'Total Revenue'
# 5) собираем бинарник: python -m cProfile -o pop.out financial_enhanced.py 'MSFT' 'Total Revenue'
# затем запускаем скрипт: python pstat_culm.py > pstats-cumulative.txt 